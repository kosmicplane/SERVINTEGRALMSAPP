 ...
 * @return [string]
 */
function atua_media_range( $css_prop, $obj_value, $default, $media = 'desktop', $ext = '' ) {
...
/**
 * Takes Ranges Control applies to atua_media_range function and ouputs Full css with @media query
 * @param  [string] $css_prop CSS Property to add
 * @param  [string] $control control / settings
 * @param  [string] $ext css value extension, eg. px, in, pc
 * @return [string]
 */
function atua_customizer_value( $control, $css_selector, $css_prop, array $default, $ext = '' ) {
    if ( $control ) {
        $control        = get_theme_mod( $control );
        $return         = '';

        if( is_string( $control ) && is_array( json_decode( $control, true ) ) ){
            $desktop_val    = atua_media_range( $css_prop, $control,  $default[0], 'desktop', $ext );
            $tablet_val     = atua_media_range( $css_prop, $control, $default[1], 'tablet', $ext );
            $mobile_val     = atua_media_range( $css_prop, $control, $default[2], 'mobile', $ext );
