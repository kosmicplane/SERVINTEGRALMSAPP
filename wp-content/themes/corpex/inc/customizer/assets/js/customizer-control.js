...
( function( $ ) {
	...
	CORPEXCustomizer = {
		...
		_initToggles: function()
		{
			...
			CORPEXControlTrigger.triggerHook( 'corpex-toggle-control', api );

			// Loop through each setting.
			$.each( CORPEXCustomizerToggles, function( settingId, toggles ) {
						...
						$.each(toggles, function( i, toggle) {
							...
							$.each(toggle.controls, function( k, controlId) {
								...
								if ( typeof parent != 'undefined' ) {
									...
									if ( ! visibleIt ) {
										$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'corpex-hide' );
									} else {
										$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'corpex-hide' );
									}
								}
							});
						});
	...
	$( function() { CORPEXCustomizer.init(); } );
