<?php
$prefix = 'g5plus_';
$is_show_top_bar = g5plus_get_meta($prefix . 'top_bar');
if (($is_show_top_bar === false) || ($is_show_top_bar === '') || ($is_show_top_bar == '-1')) {
	$is_show_top_bar = g5plus_get_option('top_bar','0');
}
if (!$is_show_top_bar) {
	return; // NOT SHOW TOP BAR
}
$top_bar_layout = g5plus_get_meta($prefix . 'top_bar_layout');
if ((!$top_bar_layout) || ($top_bar_layout === '') || ($top_bar_layout == '-1')) {
	$top_bar_layout = g5plus_get_option('top_bar_layout','top-bar-1');
}

$left_sidebar = g5plus_get_meta($prefix . 'top_left_sidebar');
if ((!$left_sidebar) || ($left_sidebar === '') || ($left_sidebar == '-1')) {
	$left_sidebar = g5plus_get_option('top_bar_left_sidebar','top_bar_left');
}

$right_sidebar = g5plus_get_meta($prefix . 'top_right_sidebar');
if ((!$right_sidebar) || ($right_sidebar === '') || ($right_sidebar == '-1')) {
	$right_sidebar = g5plus_get_option('top_bar_right_sidebar','top_bar_right');
}

$top_bar_class = array('top-bar');
$mobile_header_top_bar = g5plus_get_option('mobile_header_top_bar','0');
if ($mobile_header_top_bar == '0') {
