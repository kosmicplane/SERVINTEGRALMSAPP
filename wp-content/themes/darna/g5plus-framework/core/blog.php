	...
	function g5plus_paging_load_more() {
		...
		if (!empty($link)) :
			...
			<div class="blog-load-more-wrapper">
				<button data-href="<?php echo esc_url($link); ?>" type="button"  data-loading-text="<i class='fa fa-refresh fa-spin'></i> <?php esc_attr_e("Loading...",'g5plus-darna'); ?>" class="blog-load-more darna-button style4 size-lg" autocomplete="off">
					<i class="fa fa-plus"></i> <?php esc_html_e('Load More','g5plus-darna'); ?>
				</button>
			</div>
		<?php
		endif;
                ...
                <div class="jp-gui jp-interface">
                    <ul class="jp-controls jp-play-pause">
                        <li><a href="javascript:;" class="jp-play" tabindex="1"><i
                                    class="fa fa-play"></i> <?php _e('play', 'g5plus-darna'); ?></a></li>
                        <li><a href="javascript:;" class="jp-pause" tabindex="1"><i
                                    class="fa fa-pause"></i> <?php _e('pause', 'g5plus-darna'); ?></a></li>
                    </ul>
                    ...
                    <ul class="jp-controls jp-volume">
                        <li>
                            <a href="javascript:;" class="jp-mute" tabindex="1" title="mute"><i
                                    class="fa  fa-volume-up"></i> <?php _e('mute', 'g5plus-darna'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-unmute" tabindex="1" title="unmute"><i
                                    class="fa fa-volume-off"></i><?php _e('unmute', 'g5plus-darna'); ?></a>
                        </li>
                        <li>
                    ...
                    <ul class="jp-toggles">
                        <li>
                            <a href="javascript:;" class="jp-shuffle" tabindex="1"
                               title="shuffle"><?php _e('shuffle', 'g5plus-darna'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-shuffle-off" tabindex="1"
                               title="shuffle off"><?php _e('shuffle off', 'g5plus-darna'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-repeat" tabindex="1"
                               title="repeat"><?php _e('repeat', 'g5plus-darna'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-repeat-off" tabindex="1"
                               title="repeat off"><?php _e('repeat off', 'g5plus-darna'); ?></a>
                        </li>
                    </ul>
                </div>

                <div class="jp-no-solution">
                    <?php printf(__('<span>Update Required</span> To play the media you will need to either update your browser to a recent version or update your <a href="%s" target="_blank">Flash plugin</a>.', 'g5plus-darna'), 'http://get.adobe.com/flashplayer/'); ?>
                </div>
            </div>
        </div>
        <?php
...
if (!function_exists('g5plus_link_pages')) {
    function g5plus_link_pages() {
        wp_link_pages(array(
            'before' => '<div class="g5plus-page-links"><span class="g5plus-page-links-title">' . esc_html__('Pages:', 'g5plus-darna') . '</span>',
            'after' => '</div>',
        ...
        <li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
            <div id="comment-<?php comment_ID(); ?>" class="comment-body clearfix">
                <div class="comment-avatar-wrap">
                    ...
                    <?php comment_reply_link(array_merge($args, array('depth' => $depth, 'max_depth' => $args['max_depth'] , 'reply_text' => '<i data-toggle="tooltip" data-placement="bottom" title="'.esc_attr__('Reply','g5plus-darna').'" class="fa fa-mail-reply"></i>'))) ?>
                </div>
                <div class="comment-text">
                    ...
                    <div class="comment-meta">
                        ...
                        <?php edit_comment_link(esc_html__('Edit', 'g5plus-darna'), '<span class="comment-meta-edit">', '</span>') ?>
                    </div>
                    <div class="text"><?php comment_text() ?></div>
                    <?php if ($comment->comment_approved == '0') : ?>
                        <em><?php _e('Your comment is awaiting moderation.', 'g5plus-darna') ?></em>
                    <?php endif; ?>
                </div>
            </div>
    <?php
    ...
    function g5plus_comment_form() {
        ...
        $fields = array(
            'author' => '<div class="form-group col-md-12">' .
                '<label for="author">'.esc_html__('Name*','g5plus-darna').'</label>'.
            ...
            'email' => '<div class="form-group col-md-12">' .
                '<label for="email">'.esc_html__('Email*','g5plus-darna').'</label>'.
            ...
            'url'   => '<div class="form-group col-md-12">'.
                '<label for="url">'.esc_html__("Website",'g5plus-darna').'</label>'.
                '<input id="url" name="url" ' . ( $html5 ? 'type="url"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_url'] ) . '" placeholder="'.esc_attr__('Website','g5plus-darna').'" />'.
                '</div>'
        );
        ...
        $comment_form_args = array(
            ...
            'comment_field' => '<div class="form-group col-md-12">' .
                '<label for="comment">'.esc_html__('Comment','g5plus-darna').'</label>'.
            ...
            'comment_notes_before' => '<p class="comment-notes">' .
                esc_html__('Your email address will not be published.', 'g5plus-darna') /* . ( $req ? $required_text : '' ) */ .
                '</p>',
            'comment_notes_after' => '',
