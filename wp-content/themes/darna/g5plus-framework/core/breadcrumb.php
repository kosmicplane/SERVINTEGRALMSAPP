...
if (!function_exists('g5plus_get_breadcrumb_items')) {
	function g5plus_get_breadcrumb_items() {
		...
		/* Front page. */
		if (is_front_page()) {
			$item['last'] = esc_html__('Home', 'g5plus-darna' );
		}
		...
		elseif (is_singular()) {
			...
			if (is_category() || is_tag() || is_tax()) {
				...
				if (is_day())
					$item['last'] = esc_html__('Archives for ', 'g5plus-darna' ) . get_the_time('F j, Y');

				elseif (is_month())
					$item['last'] = esc_html__('Archives for ', 'g5plus-darna' ) . single_month_title(' ', false);

				elseif (is_year())
					$item['last'] = esc_html__('Archives for ', 'g5plus-darna' ) . get_the_time('Y');
			} else if (is_author())
				$item['last'] = esc_html__('Archives by: ', 'g5plus-darna' ) . get_the_author_meta('display_name', $wp_query->post->post_author);

		} /* If viewing search results. */
		else if (is_search())
			$item['last'] = esc_html__('Search results for "', 'g5plus-darna' ) . stripslashes(strip_tags(get_search_query())) . '"';

		/* If viewing a 404 error page. */
		else if (is_404())
			$item['last'] = esc_html__('Page Not Found', 'g5plus-darna' );
