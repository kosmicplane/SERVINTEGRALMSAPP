...
if ( !class_exists('XMenu_Setting_Options' ) ):
	class XMenu_Setting_Options {
		...
		function __construct() {
			...
			$this->setting_options = array(
				'settings' => array(
					...
					'config' => array(
						'setting-heading' => array(
							'text' => esc_html__('Settings','g5plus-darna'),
							'type' => 'heading',
						),

						/*'setting-responsive-breakpoint' => array(
							'text' => esc_html__('Responsive Breakpoint','g5plus-darna'),
							'type' => 'text',
							'std' => '',
						...
						'transition' => array(
							'text' => esc_html__('Select Menu Transition','g5plus-darna'),
							'type' => 'select',
							'std' => 'none',
							'options' => xmenu_get_transition()
						),
						'transition-duration' => array(
							'text' => esc_html__('Transition Duration','g5plus-darna'),
							'type' => 'text',
							'std' => '',
							'des' => esc_html__('The transition duration. By default 0.5s','g5plus-darna'),
						),
					),
				),
				...
				'images' => array(
					...
					'config' => array(
						'image-heading' => array(
							'text' => esc_html__('Images Settings','g5plus-darna'),
							'type' => 'heading',
						),
						'image-size' => array(
						...
						'image-width' => array(
							...
							'type' => 'text',
							'std' => '',
							'des' => esc_html__('The width attribute value for item images! Do not include units','g5plus-darna'),
						),
						'image-height' => array(
							'text' => esc_html__('Image Height','g5plus-darna'),
							'type' => 'text',
							'std' => '',
							'des' => esc_html__('The height attribute value for item images! Do not include units','g5plus-darna'),
						),
					),
				),
		...
		function admin_menu() {
			add_theme_page(
				__('XMENU Settings','g5plus-darna'),
				__('XMENU Settings','g5plus-darna'),
				'manage_options',
				'xmenu-settings',
		...
		function plugin_page() {
			...
			$terms = get_categories(array('taxonomy' => 'nav_menu', 'hide_empty' => false ));
			$setting_menus = array();
			...
			<div class="xmenu-settings">
				<h2>
					<a class="tab <?php echo esc_attr($current_menu == '' ? 'active' : '') ?>" href="themes.php?page=xmenu-settings"><strong><?php _e('XMENU','g5plus-darna') ?></strong> <?php _e('Setting','g5plus-darna') ?></a>
					<?php foreach ($setting_menus as $setting_menu): ?>
						<a class="tab <?php echo esc_attr($current_menu == $setting_menu ? 'active' : '') ?>" href="<?php echo esc_url('themes.php?page=xmenu-settings&menu=' . $setting_menu) ?>"><strong><?php echo esc_html($setting_menu); ?></strong> <?php _e('Setting','g5plus-darna') ?></a>
					<?php endforeach; ?>
					<a class="tab" href="themes.php?page=xmenu-settings&do=widget"><strong><?php _e('XMENU','g5plus-darna') ?></strong> <?php _e('Widget Manager','g5plus-darna') ?></a>
					<button id="setting-add-menu" type="button" class="button button-primary x-fr"><?php _e('Add Menu Setting','g5plus-darna') ?></button>
				</h2>
				<form method="post" action="themes.php?page=xmenu-settings" novalidate="novalidate" id="xmenu_settings">
					...
					<p class="submit">
						...
						<button type="button" id="xmenu-save-setting" class="button button-primary"><i class="fa fa-save"></i> <?php echo esc_html__('Save Changes','g5plus-darna')?></button>
						<?php if (!empty($current_menu)): ?>
							<input id="xmenu_menu_slug" type="hidden" name="menu_slug" value="<?php echo esc_attr($current_menu) ?>"/>
							<button type="button" class="button button-primary" id="xmenu-delete-setting"><i class="fa fa-remove"></i> <?php echo esc_html__('Delete Settings','g5plus-darna')?></button>
						<?php endif;?>
					</p>
				</form>
				<div class="x-popup" id="xmenu-setting-popup">
					<div class="x-popup-inner">
						<h3><?php _e('Select Menu for your new settings','g5plus-darna') ?> <i id="setting-add-menu-close" class="fa fa-close"></i></h3>
						<div class="x-popup-select">
							...
							<button id="xmenu-create-button" type="button"><?php _e('Create Menu Setting','g5plus-darna') ?></button>
						</div>
					</div>
				</div>
			</div>
		...
		function widget_manager() {
			...
			<div class="xmenu-settings">
				<h2>
					<a class="tab" href="themes.php?page=xmenu-settings"><strong><?php _e('XMENU','g5plus-darna') ?></strong> <?php _e('Setting','g5plus-darna') ?></a>
					<?php foreach ($setting_menus as $setting_menu): ?>
						<a class="tab" href="<?php echo esc_url('themes.php?page=xmenu-settings&menu=' . $setting_menu)  ?>"><strong><?php echo esc_html($setting_menu); ?></strong> <?php _e('Setting','g5plus-darna') ?></a>
					<?php endforeach; ?>
					<a class="tab active" href="themes.php?page=xmenu-settings&do=widget"><strong><?php _e('XMENU','g5plus-darna') ?></strong> <?php _e('Widget Manager','g5plus-darna') ?></a>
				</h2>
						...
						<tbody>
							<?php foreach( $widgets as $menu_item_id => $widget_title ):?>
								<tr>
									...
									<td><?php echo esc_html($widget_title) . esc_html__(' [Menu Item ', 'g5plus-darna') . esc_html($menu_item_id) .']'; ?></td>
								</tr>
							<?php endforeach;?>
						</tbody>
					</table>
					<p><?php _e('Choose which Custom Widget Areas to delete', 'g5plus-darna') ?></p>
					<p class="submit"><input type="submit" name="submit" id="submit" class="button button-primary" value="<?php echo esc_attr__('Delete Widget Areas','g5plus-darna')?>"></p>
				</form>
			</div>
			<?php
		}
		...
		function xmenu_setting_create() {
			$error_result = array(
				'code' => '-1',
				'message' => esc_html__('Create menu settings fail','g5plus-darna'),
			);
		...
		function xmenu_delete_setting() {
			$error_result = array(
				'code' => '-1',
				'message' => esc_html__('Delete menu settings fail','g5plus-darna'),
			);
