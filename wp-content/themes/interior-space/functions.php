 ...
 */
function interior_space_theme_setup(){
    /*
     * Make child theme available for translation.
     * Translations can be filed in the /languages/ directory.
     */
    load_child_theme_textdomain( 'interior-space', get_stylesheet_directory() . '/languages' );	
	require get_stylesheet_directory() . '/inc/customizer/interior-space-customizer-options.php';	
}
add_action( 'after_setup_theme', 'interior_space_theme_setup' );

/**
 * Load assets.
 */

function interior_space_theme_css() {
	wp_enqueue_style( 'interior-space-parent-style', get_template_directory_uri() . '/style.css' );
	wp_enqueue_style('interior-space-child-style', get_stylesheet_directory_uri() . '/style.css');
	wp_enqueue_style('interior-space-default-css', get_stylesheet_directory_uri() . "/assets/css/theme-default.css" );
    wp_enqueue_style('interior-space-bootstrap-smartmenus-css', get_stylesheet_directory_uri() . "/assets/css/bootstrap-smartmenus.css" ); 	
}
add_action( 'wp_enqueue_scripts', 'interior_space_theme_css', 99);

/**
 * Import Options From Parent Theme
 *
 */
function interior_space_parent_theme_options() {
...
add_action( 'after_switch_theme', 'interior_space_parent_theme_options' );

/**
 * Fresh site activate
 *
 */
$fresh_site_activate = get_option( 'fresh_interior_space_site_activate' );
if ( (bool) $fresh_site_activate === false ) {
	...
	set_theme_mod( 'arilewp_theme_color', 'theme-selective-yellow' );
	...
	update_option( 'fresh_interior_space_site_activate', true );
}

/**
 * Custom Theme Script
*/
function interior_space_custom_theme_css() {
	$interior_space_testomonial_background_image = get_theme_mod('arilewp_testomonial_background_image');
	?>
    <style type="text/css">
		<?php if($interior_space_testomonial_background_image != null) : ?>
		.theme-testimonial { 
		        background-image: url(<?php echo esc_url( $interior_space_testomonial_background_image ); ?>); 
...
<?php }
add_action('wp_footer','interior_space_custom_theme_css');

/**
 * Page header
 *
 */
function interior_space_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'interior_space_custom_header_args', array(
		...
		'flex-height'        => true,
		'flex-width'         => true,
		'wp-head-callback'   => 'interior_space_header_style',
	) ) );
}

add_action( 'after_setup_theme', 'interior_space_custom_header_setup' );

/**
 * Custom background
 *
 */
function interior_space_custom_background_setup() {
	add_theme_support( 'custom-background', apply_filters( 'interior_space_custom_background_args', array(
		'default-color' => '202020',
		'default-image' => '',
	) ) );
}
add_action( 'after_setup_theme', 'interior_space_custom_background_setup' );


if ( ! function_exists( 'interior_space_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see interior_space_custom_header_setup().
	 */
	function interior_space_header_style() {
