/* global jQuery */
/* global wp */
function accron_media_upload(button_class) {
    'use strict';
 ...
 *** Generate unique id ***
 *********************************************/
function accron_customizer_repeater_uniqid(prefix, more_entropy) {
    'use strict';
 ...
 *** General Repeater ***
 *********************************************/
function accron_customizer_repeater_refresh_social_icons(th) {
    'use strict';
    ...
    th.find('.customizer-repeater-social-repeater-container').each(function () {
        ...
        if (!id) {
            id = 'customizer-repeater-social-repeater-' + accron_customizer_repeater_uniqid();
    ...
    accron_customizer_repeater_refresh_general_control_values();
}


function accron_customizer_repeater_refresh_general_control_values() {
    'use strict';
    jQuery('.customizer-repeater-general-control-repeater').each(function () {
        ...
        th.find('.customizer-repeater-general-control-repeater-container').each(function () {
            ...
            if (!id) {
                id = 'social-repeater-' + accron_customizer_repeater_uniqid();
            ...
            if (text !== '' || image_url !== ''  || image_url2 !== '' || title !== '' || subtitle !== '' || icon_value !== '' || link !== '' || choice !== '' || social_repeater !== '' || shortcode !== '' || slide_align !== '' || color !== '') {
                values.push({
                    ...
                    'color': color,
                    'color2': color2,
                    'text': accronescapeHtml(text),
                    'link': link,
                    'text2': accronescapeHtml(text2),
                    'link2': link2,
                    'image_url': (choice === 'customizer_repeater_none' ? '' : image_url),
                    'choice': choice,
					'image_url2': image_url2,
                    'title': accronescapeHtml(title),
                    'subtitle': accronescapeHtml(subtitle),
					'subtitle2': accronescapeHtml(subtitle2),
					'subtitle3': accronescapeHtml(subtitle3),
					'subtitle4': accronescapeHtml(subtitle4),
					'subtitle5': accronescapeHtml(subtitle5),
					'slide_align': accronescapeHtml(slide_align),
                    'social_repeater': accronescapeHtml(social_repeater),
                    'id': id,
                    'shortcode': accronescapeHtml(shortcode)
                });
            }
...
jQuery(document).ready(function () {
    'use strict';
    var accron_theme_controls = jQuery('#customize-theme-controls');
    accron_theme_controls.on('click', '.customizer-repeater-customize-control-title', function () {
    ...
    accron_theme_controls.on('change', '.icp',function(){
        accron_customizer_repeater_refresh_general_control_values();
        return false;
    });
	
	accron_theme_controls.on('change','.customizer-repeater-slide-align', function(){
		accron_customizer_repeater_refresh_general_control_values();
        return false;
		
	});
	
	accron_theme_controls.on('change','.customizer-repeater-image2-control', function(){
		accron_customizer_repeater_refresh_general_control_values();
        return false;
    ...
    accron_theme_controls.on('change', '.customizer-repeater-image-choice', function () {
    ...
    accron_media_upload('.customizer-repeater-custom-media-button');
     ...
     *
     */
    accron_theme_controls.on('click', '.customizer-repeater-new-field', function () {
        var split_add_more_button=jQuery(this).text();
        var split_add_more_button_split=split_add_more_button.substr(4, 12);
        var th = jQuery(this).parent();
        var id = 'customizer-repeater-' + accron_customizer_repeater_uniqid();
		
		if(split_add_more_button_split=="Add New Slid")
        {
        if(jQuery('#exist_accron_Slider').val()>=1)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_accron_Slider').val()<1)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Slider').val())+1;
         jQuery('#exist_accron_Slider').val(new_service_add_val);  
        }
        }
		
		 if(split_add_more_button_split=="Add New Serv")
        {
        if(jQuery('#exist_accron_Service').val()>=4)
        {
        jQuery(".customizer_service_upgrade_section").show();
        return false;   
        }
         if(jQuery('#exist_accron_Service').val()<3)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Service').val())+1;
         jQuery('#exist_accron_Service').val(new_service_add_val);  
        }
        }
         if(split_add_more_button_split=="Add New Info")
        {
        if(jQuery('#exist_accron_Info').val()>=6)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_accron_Info').val()<6)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Info').val())+1;
         jQuery('#exist_accron_Info').val(new_service_add_val);  
        }
        }
		
		if(split_add_more_button_split=="Add New Soci")
        {
        if(jQuery('#exist_accron_Social').val()>=5)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_accron_Social').val()<5)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Social').val())+1;
         jQuery('#exist_accron_Social').val(new_service_add_val);  
        }
        }
		
		if(split_add_more_button_split=="Add New Test")
        {
        if(jQuery('#exist_accron_Testimonial').val()>=3)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_accron_Testimonial').val()<3)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Testimonial').val())+1;
         jQuery('#exist_accron_Testimonial').val(new_service_add_val);  
        }
        }
		
		if(split_add_more_button_split=="Add New Menu")
        {
        if(jQuery('#exist_accron_Menus').val()>=2)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_accron_Menus').val()<2)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Menus').val())+1;
         jQuery('#exist_accron_Menus').val(new_service_add_val);  
        }
        }
		
		if(split_add_more_button_split=="Add New Clie")
        {
        if(jQuery('#exist_accron_Client').val()>=5)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_accron_Client').val()<5)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Client').val())+1;
         jQuery('#exist_accron_Client').val(new_service_add_val);  
        }
        }		
		
		if(split_add_more_button_split=="Add New Feat")
        {
        if(jQuery('#exist_accron_Features').val()>=8)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_accron_Features').val()<8)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Features').val())+1;
         jQuery('#exist_accron_Features').val(new_service_add_val);  
        }
        }

		if(split_add_more_button_split=="Add New Team")
        {
        if(jQuery('#exist_accron_Team').val()>=4)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_accron_Team').val()<4)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Team').val())+1;
         jQuery('#exist_accron_Team').val(new_service_add_val);  
        }
        }

		if(split_add_more_button_split=="Add New Funf")
        {
        if(jQuery('#exist_accron_Funfact').val()>=4)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_accron_Funfact').val()<4)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Funfact').val())+1;
         jQuery('#exist_accron_Funfact').val(new_service_add_val);  
        }
        }

		if(split_add_more_button_split=="Add New Proc")
        {
        if(jQuery('#exist_accron_Process').val()>=4)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_accron_Process').val()<4)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Process').val())+1;
         jQuery('#exist_accron_Process').val(new_service_add_val);  
        }
        }
		
		if(split_add_more_button_split=="Add New Port")
        {
        if(jQuery('#exist_accron_Portfolio').val()>=5)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_accron_Portfolio').val()<5)
        {
         var new_service_add_val=parseInt(jQuery('#exist_accron_Portfolio').val())+1;
         jQuery('#exist_accron_Portfolio').val(new_service_add_val);  
        }
        }
    ...
    accron_theme_controls.on('click', '.social-repeater-general-control-remove-field', function () {
        ...
        if (typeof    jQuery(this).parent() !== 'undefined') {
            jQuery(this).parent().hide(500, function(){
                ...
                {
                jQuery('#exist_accron_Slider').val(parseInt(jQuery('#exist_accron_Slider').val())-1);  
                }
				if(split_delete_button_split=="Delete Servi")
                {
                jQuery('#exist_accron_Service').val(parseInt(jQuery('#exist_accron_Service').val())-1);  
                }
				if(split_delete_button_split=="Delete Conte")
                {
                jQuery('#exist_accron_Info').val(parseInt(jQuery('#exist_accron_Info').val())-1); 
                }
				if(split_delete_button_split=="Delete Socia")
                {
                jQuery('#exist_accron_Social').val(parseInt(jQuery('#exist_accron_Social').val())-1); 
                }
				if(split_delete_button_split=="Delete Testi")
                {
                jQuery('#exist_accron_Testimonial').val(parseInt(jQuery('#exist_accron_Testimonial').val())-1); 
                }
				if(split_delete_button_split=="Delete Menus")
                {
                jQuery('#exist_accron_Menus').val(parseInt(jQuery('#exist_accron_Menus').val())-1); 
                }
				if(split_delete_button_split=="Delete Clien")
                {
                jQuery('#exist_accron_Client').val(parseInt(jQuery('#exist_accron_Client').val())-1); 
                }				
				if(split_delete_button_split=="Delete Featu")
                {
                jQuery('#exist_accron_Features').val(parseInt(jQuery('#exist_accron_Features').val())-1); 
                }
				if(split_delete_button_split=="Delete Team")
                {
                jQuery('#exist_accron_Team').val(parseInt(jQuery('#exist_accron_Team').val())-1); 
                }
				if(split_delete_button_split=="Delete Funfa")
                {
                jQuery('#exist_accron_Funfact').val(parseInt(jQuery('#exist_accron_Funfact').val())-1); 
                }
				if(split_delete_button_split=="Delete Proce")
                {
                jQuery('#exist_accron_Process').val(parseInt(jQuery('#exist_accron_Process').val())-1); 
                }
				if(split_delete_button_split=="Delete Portf")
                {
                jQuery('#exist_accron_Portfolio').val(parseInt(jQuery('#exist_accron_Portfolio').val())-1); 
                }
    ...
    accron_theme_controls.on('keyup', '.customizer-repeater-title-control', function () {
        accron_customizer_repeater_refresh_general_control_values();
    });
    ...
    accron_theme_controls.on('keyup', '.customizer-repeater-subtitle-control', function () {
        accron_customizer_repeater_refresh_general_control_values();
    });
	
	 accron_theme_controls.on('keyup', '.customizer-repeater-subtitle2-control', function () {
        accron_customizer_repeater_refresh_general_control_values();
    });
	
	accron_theme_controls.on('keyup', '.customizer-repeater-subtitle3-control', function () {
        accron_customizer_repeater_refresh_general_control_values();
    });
	
	accron_theme_controls.on('keyup', '.customizer-repeater-subtitle4-control', function () {
        accron_customizer_repeater_refresh_general_control_values();
    });
	
	accron_theme_controls.on('keyup', '.customizer-repeater-subtitle5-control', function () {
        accron_customizer_repeater_refresh_general_control_values();
    });

    accron_theme_controls.on('keyup', '.customizer-repeater-shortcode-control', function () {
        accron_customizer_repeater_refresh_general_control_values();
    });

    accron_theme_controls.on('keyup', '.customizer-repeater-text-control', function () {
        accron_customizer_repeater_refresh_general_control_values();
    });

    accron_theme_controls.on('keyup', '.customizer-repeater-link-control', function () {
        accron_customizer_repeater_refresh_general_control_values();
    });

    accron_theme_controls.on('keyup', '.customizer-repeater-text2-control', function () {
        accron_customizer_repeater_refresh_general_control_values();
    });

    accron_theme_controls.on('keyup', '.customizer-repeater-link2-control', function () {
        accron_customizer_repeater_refresh_general_control_values();
    });
    ...
    /*----------------- Socials Repeater ---------------------*/
    accron_theme_controls.on('click', '.social-repeater-add-social-item', function (event) {
        event.preventDefault();
        var th = jQuery(this).parent();
        var id = 'customizer-repeater-social-repeater-' + accron_customizer_repeater_uniqid();
        if (typeof th !== 'undefined') {
    ...
    accron_theme_controls.on('click', '.social-repeater-remove-social-item', function (event) {
        event.preventDefault();
        var th = jQuery(this).parent();
        var repeater = jQuery(this).parent().parent();
        th.remove();
        accron_customizer_repeater_refresh_social_icons(repeater);
        return false;
    });

    accron_theme_controls.on('keyup', '.customizer-repeater-social-repeater-link', function (event) {
        event.preventDefault();
        var repeater = jQuery(this).parent().parent();
        accron_customizer_repeater_refresh_social_icons(repeater);
        return false;
    ...
    accron_theme_controls.on('change', '.customizer-repeater-social-repeater-container .icp', function (event) {
        event.preventDefault();
...
function accronescapeHtml(string) {
    'use strict';
    //noinspection JSUnresolvedFunction
    string = String(string).replace(new RegExp('\r?\n', 'g'), '<br />');
    string = String(string).replace(/\\/g, '&#92;');
    return String(string).replace(/[&<>"'\/]/g, function (s) {
        return accronentityMap[s];
    });
