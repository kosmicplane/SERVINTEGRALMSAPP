...
( function( $ ) {
	...
	ACCRONCustomizer = {
		...
		_initToggles: function()
		{
			...
			ACCRONControlTrigger.triggerHook( 'accron-toggle-control', api );

			// Loop through each setting.
			$.each( ACCRONCustomizerToggles, function( settingId, toggles ) {
						...
						$.each(toggles, function( i, toggle) {
							...
							$.each(toggle.controls, function( k, controlId) {
								...
								if ( typeof parent != 'undefined' ) {
									...
									if ( ! visibleIt ) {
										$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'accron-hide' );
									} else {
										$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'accron-hide' );
									}
								}
							});
						});
	...
	$( function() { ACCRONCustomizer.init(); } );
