 ...
 * @access public
 */
class Accron_Customizer_Range_Control extends WP_Customize_Control {
	...
	/**
	 * Accron_Customizer_Range_Control constructor.
	 *
 ...
 */
function accron_pro_sanitize_range_value( $input ) {
	if ( accron_pro_is_json( $input ) ) {
 ...
 * @param  string $string Input.
 */
function accron_pro_is_json( $string ) {
	return is_string( $string ) && is_array( json_decode( $string, true ) ) ? true : false;
}
