...
class RWMB_Taxonomy_Field extends RWMB_Object_Choice_Field {
	...
	public static function normalize( $field ) {
		...
		if ( empty( $field['placeholder'] ) ) {
			$field['placeholder'] = __( 'Select a term', 'g5plus-darna' );
			if ( is_string( $field['taxonomy'] ) && taxonomy_exists( $field['taxonomy'] ) ) {
				$taxonomy_object = get_taxonomy( $field['taxonomy'] );

				// Translators: %s is the taxonomy singular label.
				$field['placeholder'] = sprintf( __( 'Select a %s', 'g5plus-darna' ), $taxonomy_object->labels->singular_name );
			}
		}
	...
	public static function get_options( $field ) {
		$args = wp_parse_args($field['query_args'],array('taxonomy' => $field['taxonomy']));
