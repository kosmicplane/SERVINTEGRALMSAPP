	...
	class G5Plus_Install_Demo
	{
		...
		function setting_assets($hook)
		{
			if ($hook == 'appearance_page_install-demo') {
				wp_enqueue_style('g5plus-install-demo-data', PLUGIN_G5PLUS_FRAMEWORK_URL . '/g5plus-framework/install-demo/assets/css/admin.css');
				wp_enqueue_script('g5plus-install-demo-data', PLUGIN_G5PLUS_FRAMEWORK_URL . '/g5plus-framework/install-demo/assets/js/app.js', false, true);
		...
		function  install_demo() {
			if (!(isset($_REQUEST['security']) && current_user_can( 'manage_options' )) )
			{
				...
				$data_response = array(
					'code' => 'error',
					'message' => esc_html__("Permission error!",'g5plus-darna')
				);
			...
			$import_file_path    = G5PLUS_FRAMEWORK_THEME_DIR  . "assets". DIRECTORY_SEPARATOR ."data-demo" . DIRECTORY_SEPARATOR . $demo_site . DIRECTORY_SEPARATOR ."demo-data.xml";
			$import_setting_path = G5PLUS_FRAMEWORK_THEME_DIR  . "assets". DIRECTORY_SEPARATOR ."data-demo" . DIRECTORY_SEPARATOR . $demo_site . DIRECTORY_SEPARATOR ."setting.json";
			...
			if ( ! class_exists( 'G5_Import' ) ) {
				$class_wp_import = PLUGIN_G5PLUS_FRAMEWORK_DIR . 'g5plus-framework/install-demo/wordpress-importer.php';
			...
			if ($importer_error !== false) {
				...
				$data_response = array(
					'code' => 'fileNotFound',
					'message' => esc_html__("The Auto importing script could not be loaded. please use the wordpress importer and import the XML file that is located in your themes folder manually.",'g5plus-darna')
				);
				echo json_encode($data_response);
				die();
			}
				...
				if ( class_exists( 'G5_Import' ) ) {
					include_once( PLUGIN_G5PLUS_FRAMEWORK_DIR . 'g5plus-framework/install-demo/g5plus_import_class.php' );
				}
				...
				switch (trim($type)) {
					case 'init':
						$demo_data_directory = G5PLUS_FRAMEWORK_THEME_DIR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . $demo_site . DIRECTORY_SEPARATOR;
						$arr_demo_file = array(
						...
						foreach ( $arr_demo_file as $file_demo ) {
							if (!file_exists($file_demo)) {
								...
								$data_response = array(
									'code' => 'fileNotFound',
									'message' => esc_html__("File not found! Please check file exists in directory:\n[your-theme]/assets/data-demo/",'g5plus-darna') . $demo_site
								);
								echo json_encode($data_response);
								die();
							}
						}
						if ( $handle = opendir( G5PLUS_FRAMEWORK_THEME_DIR . "assets" . DIRECTORY_SEPARATOR . "data-demo" . DIRECTORY_SEPARATOR . DIRECTORY_SEPARATOR . "log" ) ) {
							...
							while ( false !== ( $entry = readdir( $handle ) ) ) {
								...
								if ( $entry != "." && $entry != ".." ) {
									unlink( G5PLUS_FRAMEWORK_THEME_DIR . "assets" . DIRECTORY_SEPARATOR . "data-demo". DIRECTORY_SEPARATOR . "log" . DIRECTORY_SEPARATOR . $entry );
								}
							}
						}
					...
					case 'slider':
						...
						do_action('g5plus_install_demo_data_done');
					...
					case 'fix-data':
						...
						foreach ( $arr_demo_file as $file_demo ) {
							if (!file_exists($file_demo)) {
								...
								$data_response = array(
									'code' => 'fileNotFound',
									'message' => esc_html__("File not found! Please check file exists in directory:\n[your-theme]/assets/data-demo/",'g5plus-darna')
								);
								echo json_encode($data_response);
								die();
							}
		...
		function control_panel() {
			include PLUGIN_G5PLUS_FRAMEWORK_DIR . 'g5plus-framework/install-demo/template/install-demo-page.php';
		}
