...
if (!class_exists('g5plusFramework_Shortcode_Product')) {
    class g5plusFramework_Shortcode_Product {
        function __construct() {
            add_shortcode('darna_product', array($this, 'product_shortcode' ));
        }

        function  product_shortcode($atts) {
            $title =  $source = $filter = $category = $per_page = $columns = $slider = $autoplay = $orderby = $order = $el_class = $css_animation = $duration = $delay =  '';
            ...
            if (in_array($filter,array('sale','featured')) || $source == 'category') {
	        ...
	        $g5plus_woocommerce_loop['autoPlay'] = !empty($autoplay) ? 'true' : 'false' ;
            ob_start();
            ?>
            ...
            <?php else: ?>
                <div class="item-not-found"><?php echo __('No item found','g5plus-darna') ?></div>
            <?php endif; ?>
