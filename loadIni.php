<?php
    ...
    foreach ($names as $index => $name) {
        ...
        if ($sizes[$index] <= 0 || $sizes[$index] > $maxSize) {
            respond_with_result('error', 'El archivo supera el tamaño permitido (5MB).');
        }
        ...
        if (!@move_uploaded_file($tmpNames[$index], $target_path)) {
            respond_with_result('error', 'No fue posible guardar el archivo. Consulte con soporte.');
        }
